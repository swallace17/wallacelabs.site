...
### Installing Docker
Our goal here is to setup your server to act as a Docker host. The Operating System (OS) you choose is mostly up to your preference, as Docker can be run on Windows, macOS, or Linux.It is, however, worth mentioning that less setup is required on Linux or macOS. On Windows, you will need to install Docker Desktop, as stated below, but you will also need to configure Windows Subsystem for Linux (WSL) and a Linux Distribution of your choice to run on WSL. This is not overly complicated, but it is outside the scope of what I'll be addressing here. A post for another time, perhaps. 
